...
//        self.inputToolbar.contentView.rightBarButtonItem.setBackgroundImage(UIImage(named: "Send-41") as UIImage?, for:.disabled)
        ...
        self.inputToolbar.contentView.textView.translatesAutoresizingMaskIntoConstraints = false
       ...
       // print(self.senderId)
        ...
        let childRef = Database.database().reference().child("chat").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!).queryLimited(toLast:self.start_index)
            ...
            if snapshot.childrenCount > 0 {
                ...
                for artists in snapshot.children.allObjects as! [DataSnapshot] {
                    ...
                    if(self.senderId != id as? String ){
                        if(status == "0"){
                            let childRef23 = Database.database().reference().child("chat").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
...
//                        self.collectionView.scrollToItem(at: indexPath as IndexPath, at: .bottom, animated: true)
        ...
        let childRef25 = Database.database().reference().child("chat").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!).queryLimited(toLast:self.start_index)
...
//                                        self.seenStatus.add(status)
                    if(self.senderId != id as? String ){
                        if(status == "0"){
                            ...
                            let childRef24 = Database.database().reference().child("chat").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        let childRef = Database.database().reference().child("typing_indicator").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
        let childRef1 = Database.database().reference().child("typing_indicator").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
    ...
    override func textViewDidChange(_ textView: UITextView) {
        ...
        if(textView.text == nil || textView.text == "" || textView.text == " "){
            ...
            let message = ["sender_id":UserDefaults.standard.string(forKey:"uid")!,"receiver_id":StaticData.obj.receiver_id]
...
//        self.navigationController?.view.addSubview(view1)
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: String!, senderDisplayName: String!, date: Date!)
    {
        ...
        let childRef = Database.database().reference().child("chat").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
        let childRef1 = Database.database().reference().child("chat").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
...
//        let message = ["sender_id":UserDefaults.standard.string(forKey:"uid")!,"receiver_id":StaticData.singleton.receiver_id,"sender_name":UserDefaults.standard.string(forKey: "first_name")!+" "+UserDefaults.standard.string(forKey: "last_name")!,"text":text,"timestamp":result,"status":"0","time":""]
        let message = ["sender_id":UserDefaults.standard.string(forKey:"uid")!,"receiver_id":StaticData.obj.receiver_id,"sender_name":UserDefaults.standard.string(forKey:"Username")!,"text":text,"timestamp":result,"status":"0","time":"","type":"text","pic_url":"","chat_id":key]
        
        
        childRef.child(key!).setValue(message)
        childRef1.child(key!).setValue(message)
        
        let childRef22 = Database.database().reference().child("typing_indicator").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
        let childRef222 = Database.database().reference().child("typing_indicator").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
        self.showTypingIndicator = false
        
        childRef22.removeValue()
        childRef222.removeValue()
       
        let inbox = ["date":result,"msg":text,"timestamp":result,"rid":StaticData.obj.receiver_id!,"status":"1","name":StaticData.obj.receiver_name!,"pic":StaticData.obj.receiver_img!]
        ...
        self.childRef30.child(UserDefaults.standard.string(forKey:"uid")!).child(StaticData.obj.receiver_id!).updateChildValues(inbox as [AnyHashable : Any])
        
        self.childRef31.child(StaticData.obj.receiver_id!).child(UserDefaults.standard.string(forKey:"uid")!).updateChildValues(inbox1 as [AnyHashable : Any])
       
        self.childRef32.child(UserDefaults.standard.string(forKey:"uid")!).child(StaticData.obj.receiver_id!).removeValue()
    
            Database.database().reference().child("Users").child(StaticData.obj.receiver_id!).observeSingleEvent(of: .value) {
                (snapshot) in
                 let firebaseDic = snapshot.value as? [String: AnyObject]
                if let phoneDB = firebaseDic?["token"] as? String {
                    ...
                    Database.database().reference().child("notifications").child(UserDefaults.standard.string(forKey:"uid")!).child(notkey!).updateChildValues([
                        "name":UserDefaults.standard.string(forKey:"first_name")!+" "+UserDefaults.standard.string(forKey:"last_name")!,"message":text,"picture":"https://graph.facebook.com/1126252230860258/picture?width=500&width=500","token":phoneDB,"receiverid":StaticData.obj.receiver_id!])
                }
        }
...
//        let url : String = StaticData.obj.baseUrl!+"sendPushNotification"
//
//
//        let parameter:[String:Any]?  = ["tokon":UserDefaults.standard.string(forKey:"DeviceToken")!,"title":UserDefaults.standard.string(forKey:"Username")!,"message":text,"icon":UserDefaults.standard.string(forKey:"image_url")!,"senderid":UserDefaults.standard.string(forKey:"uid")!,"receiverid":StaticData.obj.receiver_id!,"action_type":"message"]
//
//        print(url)
//        print(parameter!)
//
...
//        if(StaticData.obj.matchORinbox == "match"){
//            self.CallApi()
//        }else{
//
//        }
        ...
        if let uploadData = pickedImage!.jpeg(.lowest) {
            ...
            storageRef.putData(uploadData, metadata: nil, completion: { (metadata,error ) in
                ...
                storageRef.downloadURL { (url, error) in
                    ...
                    let childRef = Database.database().reference().child("chat").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
                    _ = Database.database().reference().child("chat").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
                    ...
                    let message = ["sender_id":UserDefaults.standard.string(forKey:"uid")!,"receiver_id":StaticData.obj.receiver_id,"sender_name":UserDefaults.standard.string(forKey:"Username")!,"text":"","timestamp":result,"status":"0","time":"","type":"image","pic_url":self.downloadURL,"chat_id":key]
                    
                    let childRef2 = Database.database().reference().child("typing_indicator").child(UserDefaults.standard.string(forKey:"uid")!+"-"+StaticData.obj.receiver_id!)
                    let childRef1 = Database.database().reference().child("typing_indicator").child(StaticData.obj.receiver_id!+"-"+UserDefaults.standard.string(forKey:"uid")!)
                    let inbox = ["date":result,"msg":"send an image...","timestamp":result,"rid":StaticData.obj.receiver_id!,"status":"1","name":StaticData.obj.receiver_name!,"pic":StaticData.obj.receiver_img!]
                    ...
                    Database.database().reference().child("Users").child(StaticData.obj.receiver_id!).observeSingleEvent(of: .value) {
                        ...
                        if let phoneDB = firebaseDic?["token"] as? String {
                            ...
                            let notkey =  Database.database().reference().child("notifications").key
                            Database.database().reference().child("notifications").child(UserDefaults.standard.string(forKey:"uid")!).child(notkey!).updateChildValues([
                                "name":UserDefaults.standard.string(forKey:"first_name")!+" "+UserDefaults.standard.string(forKey:"last_name")!,"message":"sending an image....","picture":StaticData.obj.receiver_img!,"token":phoneDB,"receiverid":StaticData.obj.receiver_id!])
                        }
                    }
