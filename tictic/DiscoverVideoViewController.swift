...
class DiscoverVideoViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,UITableViewDelegate,UITableViewDataSource,UICollectionViewDataSourcePrefetching,UIGestureRecognizerDelegate,UITextViewDelegate{
...
//        }
    ...
    func updateNextSet(){
        ...
        showAllVideosList(offset:  self.offset)
    ...
    @objc func onTapCD(_ sender: UIButton) {
        if(UserDefaults.standard.string(forKey: "uid") == ""){
            ...
            vc.audioTitle = obj.first_name + "" + obj.last_name
...
//           print(parameter!)
    ...
    @objc func connected2(_ sender : UIButton) {
        ...
        let indexPath = IndexPath(row: buttonTag, section: 0)
        let cell = collectionview.cellForItem(at: indexPath) as? homecollCell
            var obj = self.friends_array[buttonTag] as! Home
...
//        cell.comment_time.text = str
    ...
    func showAllVideos(){
        ...
        let parameter :[String:Any]? = ["fb_id": self.fbId, "middle_name": self.appDelegate.middle_name, "video_id": self.videoId]
...
//                        let video_url = "https://www.radiantmediaplayer.com/media/big-buck-bunny-360p.mp4"
                    ...
                    DispatchQueue.main.async {
                        if self.friends_array.count == 0{
                            ...
                            if self.fromScreen == "Discover" || self.fromScreen == "Notification"{
                            ...
                            self.showAllVideosList(offset: 0)
                            }
                        }
    ...
    func showAllVideosList(offset: Int?){
        
        if(UserDefaults.standard.string(forKey:"DeviceToken" ) == nil){
            UserDefaults.standard.set("NULL", forKey:"DeviceToken")
        }
        let url : String = self.appDelegate.baseUrl!+self.appDelegate.showAllVideosList!
        let sv = HomeViewController.displaySpinner(onView: self.view)
        
        let parameter :[String:Any]? = ["fb_id": self.fbId, "type": self.type, "offset" : String(offset ?? 0), "section_id": self.sectionId]
            ...
            case .success( let value):
                ...
                if(code == "200"){
                    ...
                    for Dict in myCountry {
                        ...
                        let allow_duet:String! =   myRestaurant["allow_duet"] as? String
                        //    print("is follow",isFollow)
                        let obj = Home(like_count: like_count, video_comment_count: video_comment_count, sound_name: sound_name,thum: thum, first_name: first_name, last_name: last_name,profile_pic: profile_pic, video_url: video_url, v_id: v_id, u_id: u_id, like: like, desc: desc, f_name: f_name, view_count: view_count, verified: verification, allow_comment: allow_comment, allow_duet: allow_duet, isFollow: 0, share_count: shareCount)
                        var myArr : [Home] = []
                        myArr = self.friends_array as! [Home]
                        if (myArr.first(where: {$0.v_id == obj.v_id}) != nil) {
                            print("already present")
                        }else{
                            self.friends_array.add(obj)
                        }
//
//                        print(self.friends_array.count)
                    }
                    ...
                    DispatchQueue.main.async {
                        if self.friends_array.count == 0{
                            ...
                            self.collectionview.dataSource = self
                            self.collectionview.prefetchDataSource = self
                            self.collectionview.scrollToItem(at:IndexPath(item: self.position ?? 0, section: 0), at: [.top, .bottom], animated: false)
